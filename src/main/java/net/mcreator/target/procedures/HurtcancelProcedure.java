...
public class HurtcancelProcedure {
	...
	private static void execute(@Nullable Event event, DamageSource damagesource, Entity sourceentity) {
		if (damagesource == null || sourceentity == null)
			return;
		if (!(sourceentity == null) && sourceentity instanceof Player && (!sourceentity.isAlive() || new Object() {
		...
		}.checkGamemode(sourceentity)) && (damagesource.is(ResourceKey.create(Registries.DAMAGE_TYPE, new ResourceLocation("target:arrow_in_brain"))) || damagesource.is(ResourceKey.create(Registries.DAMAGE_TYPE, new ResourceLocation("target:mine")))
